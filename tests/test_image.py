...
class TestImageInstance:
    def test_image_instance(self, connect, dataset):
        image_instance = ImageInstance(dataset["abstract_image3"].id, dataset["project"].id).save()
...
class TestAbstractSlice:
    ...
    def test_abstract_slices_by_abstract_image(self, connect, dataset):
        abstract_slices = AbstractSliceCollection().fetch_with_filter("abstractimage", dataset["abstract_image"].id)
...
class TestSliceInstance:
    ...
    def test_slice_instance(self, connect, dataset):
        slice_instance = SliceInstance(dataset["project"].id, dataset["image_instance2"].id, dataset["abstract_slice2"].id).save()
    ...
    def test_slices_by_image(self, connect, dataset):
        slices = SliceInstanceCollection().fetch_with_filter("imageinstance", dataset["image_instance"].id)
